...
export default function registerRoutes(r) {
  ...
  r.post('/api/goals', async (req) => {
    ...
    await audit(b.actor ?? 'Alex Rivera', 'goal_created', 'esg_goal', id, b.name);
    return await get(`SELECT * FROM esg_goals WHERE id = ?`, [id]);
  });
  ...
  r.post('/api/carbon-transactions', async (req) => {
    ...
    await audit(b.actor ?? 'Alex Rivera', 'transaction_logged', 'carbon_transaction', id,
  ...
  r.post('/api/participations/:id/review', async (req) => {
    ...
    if (b.decision === 'approve') return await gamify.approve(id, actor);
    if (b.decision === 'reject') return await gamify.reject(id, b.reason, actor);
  ...
  r.patch('/api/compliance-issues/:id', async (req) => {
    ...
    await audit(b.actor ?? 'Alex Rivera', 'issue_status_changed', 'compliance_issue', id,
  ...
  r.post('/api/reports', async (req) => {
    ...
    await audit(b.actor ?? 'Alex Rivera', 'report_generated', 'report', id, b.title);
  ...
  r.put('/api/settings', async (req) => {
    ...
    await audit(b.actor ?? 'Alex Rivera', 'settings_updated', 'settings', null,
